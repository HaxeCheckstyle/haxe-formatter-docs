...
class EnumAbstractHelper {
	macro public static function getAllEnumValues(typePath:String):Expr {
		...
		var enumName:String = typePath.split(".").pop();
			...
			case TAbstract(_.get() => ab, _) if (ab.meta.has(":enum")):
				...
				for (field in ab.impl.get().statics.get()) {
					if (field.meta.has(":enum") && field.meta.has(":impl")) {
						...
						valueExprs.push(macro $i{enumName}.$fieldName);
					}
				}
