// Generated by Haxe 4.3.0-rc.1+5645ecc
...
class json2object_reader_BaseParser {
	...
	fromJson(jsonString,filename) {
		...
		this.errors = [];
		try {
			let json = new hxjsonast_Parser(jsonString,filename).doParse();
			this.loadJson(json);
		} catch( _g ) {
...
class Keywords {
	...
	getKeywordList() {
		...
		let h = this.words.h;
		let word_h = h;
		let word_keys = Object.keys(h);
		let word_length = word_keys.length;
		let word_current = 0;
		while(word_current < word_length) {
			let word = word_keys[word_current++];
			keywords.push(word);
		}
	...
	getSearchResults(keyword) {
		...
		while(word_current < word_length) {
			...
			pages = pages.concat(this.words.h[word]);
		}
		let _g = 0;
...
class codesamples_SampleBase {
	buildDocSamplePage(container,codeSampleName,docText,configText,fieldDef,codeSample,registry) {
		...
		let result = formatter_Formatter.format(formatter_FormatterInput.Code(codeSample,formatter_CodeOrigin.Snippet),this.configFieldRegistry.currentConfig);
...
class codesamples_config_ConfigFieldRegistry {
	...
	makeCustomConfig() {
		...
		let h = this.configFieldValues.h;
		let _g_h = h;
		let _g_keys = Object.keys(h);
		let _g_length = _g_keys.length;
		let _g_current = 0;
...
var codesamples_config_ConfigFieldType = $hxEnums["codesamples.config.ConfigFieldType"] = { __ename__:true,__constructs__:null
	,Combo: ($_=function(abstractValues) { return {_hx_index:0,abstractValues:abstractValues,__enum__:"codesamples.config.ConfigFieldType",toString:$estr}; },$_._hx_name="Combo",$_.__params__ = ["abstractValues"],$_)
	,Number: {_hx_name:"Number",_hx_index:1,__enum__:"codesamples.config.ConfigFieldType",toString:$estr}
	,Bool: {_hx_name:"Bool",_hx_index:2,__enum__:"codesamples.config.ConfigFieldType",toString:$estr}
	,Text: {_hx_name:"Text",_hx_index:3,__enum__:"codesamples.config.ConfigFieldType",toString:$estr}
};
codesamples_config_ConfigFieldType.__constructs__ = [codesamples_config_ConfigFieldType.Combo,codesamples_config_ConfigFieldType.Number,codesamples_config_ConfigFieldType.Bool,codesamples_config_ConfigFieldType.Text];
class doc_DocBase {
...
class formatter_Formatter {
	static format(input,config,lineSeparator,entryPoint,range) {
		...
		case 0:
			...
			inputData = { fileName : inputData1, content : content, config : config, lineSeparator : lineSeparator, entryPoint : entryPoint, range : range};
		...
		case 1:
			...
			let origin1 = input.origin;
			let inputData2;
			switch(origin1._hx_index) {
			case 0:
				let fileName1 = origin1.fileName;
				inputData2 = fileName1;
				break;
			case 1:
				inputData2 = "code snippet";
				break;
			}
			inputData = { fileName : inputData2, content : code1, tokenList : tokenList, tokenTree : tokenTree, config : config, lineSeparator : lineSeparator, entryPoint : entryPoint, range : range};
			return formatter_Formatter.formatInputData(inputData);
		}
	}
...
var formatter_FormatterInput = $hxEnums["formatter.FormatterInput"] = { __ename__:true,__constructs__:null
	,Code: ($_=function(code,origin) { return {_hx_index:0,code:code,origin:origin,__enum__:"formatter.FormatterInput",toString:$estr}; },$_._hx_name="Code",$_.__params__ = ["code","origin"],$_)
	,Tokens: ($_=function(tokenList,tokenTree,code,origin) { return {_hx_index:1,tokenList:tokenList,tokenTree:tokenTree,code:code,origin:origin,__enum__:"formatter.FormatterInput",toString:$estr}; },$_._hx_name="Tokens",$_.__params__ = ["tokenList","tokenTree","code","origin"],$_)
};
...
class formatter_codedata_CodeLines {
	...
	applyWrapping(config,lineSeparator) {
		...
		while(_g < _g1.length) {
			let line = _g1[_g];
			++_g;
			let wrappedCode = line.applyWrapping(config,this.parsedCode,this.indenter,lineSeparator);
...
class formatter_marker_Indenter {
	...
	calcIndent(token) {
		...
		let effectiveToken = this.findEffectiveParent(token);
		...
		case 18:
			...
			case 1:
				...
				case 20:case 21:
					return this.findEffectiveParent(parent3);
				case 22:
					let _g8 = parent3.parent.tok;
					if(_g8._hx_index == 1 && _g8.k._hx_index == 0) {
	...
	countLineBreaks(indentingTokensCandidates,indentComplexValueExpressions) {
		...
		while(_g < indentingTokensCandidates.length) {
			...
			case 16:case 18:
				let close = this.parsedCode.tokenList.getCloseToken(currentToken);
			...
			case 1:
				...
				case 0:
					if(currentToken.tok._hx_index == 20) {
			...
			case 18:
				...
				case 14:
					...
					break;
				case 16:case 20:
					let tmp = !this.parsedCode.tokenList.isNewLineBefore(prevToken);
					break;
				default:
				}
				break;
	...
	isIndentingToken(token,prevToken) {
		...
		case 12:
			let tmp1;
			let _g5 = token.parent.tok;
			if(!(_g5._hx_index == 1 && _g5.k._hx_index == 15)) {
...
class formatter_marker_MarkEmptyLines extends formatter_marker_MarkerBase {
	...
	markInterfaceEmptyLines(prevToken,prevTokenType,currToken,currTokenType,conf) {
		...
		switch(prevTokenType._hx_index) {
		case 0:
			let _g = prevTokenType.visibility;
			let _g1 = prevTokenType.isStatic;
			let _g2 = prevTokenType.isInline;
			let _g3 = prevTokenType.isOverride;
			let _g4 = prevTokenType.isFinal;
			let _g5 = prevTokenType.isExtern;
			let name = prevTokenType.name;
			prevVar = false;
			break;
		case 1:
			let _g6 = prevTokenType.visibility;
			let _g7 = prevTokenType.isStatic;
			let _g8 = prevTokenType.isInline;
			let _g9 = prevTokenType.isFinal;
			let _g10 = prevTokenType.isExtern;
			let name1 = prevTokenType.name;
			prevVar = true;
			break;
		case 2:
			let _g11 = prevTokenType.visibility;
			let _g12 = prevTokenType.isStatic;
			let _g13 = prevTokenType.getter;
			let _g14 = prevTokenType.setter;
			let name2 = prevTokenType.name;
			prevVar = true;
			break;
		case 3:
		...
		switch(currTokenType._hx_index) {
		case 0:
			let _g15 = currTokenType.visibility;
			let _g16 = currTokenType.isStatic;
			let _g17 = currTokenType.isInline;
			let _g18 = currTokenType.isOverride;
			let _g19 = currTokenType.isFinal;
			let _g20 = currTokenType.isExtern;
			let name3 = currTokenType.name;
			currVar = false;
			break;
		case 1:
			let _g21 = currTokenType.visibility;
			let _g22 = currTokenType.isStatic;
			let _g23 = currTokenType.isInline;
			let _g24 = currTokenType.isFinal;
			let _g25 = currTokenType.isExtern;
			let name4 = currTokenType.name;
			currVar = true;
			break;
		case 2:
			let _g26 = currTokenType.visibility;
			let _g27 = currTokenType.isStatic;
			let _g28 = currTokenType.getter;
			let _g29 = currTokenType.setter;
			let name5 = currTokenType.name;
			currVar = true;
			break;
		case 3:
	...
	keepExistingEmptyLines() {
		...
		while(_g < funcs.length) {
			let func = funcs[_g];
			++_g;
			let block = tokentree_TokenTreeAccessHelper.matches(tokentree_TokenTreeAccessHelper.firstChild(func),tokentree_TokenTreeDef.BrOpen);
			if(block == null) {
		...
		while(_g < typedefTokens.length) {
			...
			while(_g1 < _g2.length) {
				...
				case 19:
					...
					let _g4 = next.token.tok;
					if(_g4._hx_index == 6 && _g4.op._hx_index == 11) {
						this.noLineEndAfter(child);
					}
					let _g5 = next.token.tok;
					if(_g5._hx_index == 6 && _g5.op._hx_index == 7) {
...
class formatter_marker_MarkSameLine extends formatter_marker_MarkerBase {
	...
	markDollarSameLine() {
		...
		while(_g < tokens.length) {
			let token = tokens[_g];
			++_g;
			let brOpen = tokentree_TokenTreeAccessHelper.matches(tokentree_TokenTreeAccessHelper.firstChild(token),tokentree_TokenTreeDef.BrOpen);
			if(brOpen == null) {
				continue;
			}
			let brClose = this.getCloseToken(brOpen);
			...
			case 2:
				...
				if(_g1._hx_index == 3) {
					...
					case "is":
						...
						break;
					case "from":case "to":
						let parent = tokentree_TokenTreeAccessHelper.matches(tokentree_TokenTreeAccessHelper.parent(tokentree_TokenTreeAccessHelper.parent(token)),tokentree_TokenTreeDef.Kwd(haxeparser_Keyword.KwdAbstract));
						if(parent != null) {
		...
		if(closing) {
			...
			if(prev != null) {
				...
				case 14:
					return;
				case 15:
					let _g3 = this.config.whitespace.commaPolicy;
					if(_g3 != null) {
						switch(_g3) {
...
class formatter_marker_wrapping_MarkWrappingBase extends formatter_marker_MarkerBase {
	...
	noWrap(open,close) {
		let colon = tokentree_TokenTreeAccessHelper.matches(tokentree_TokenTreeAccessHelper.parent(tokentree_TokenTreeAccessHelper.matches(open,tokentree_TokenTreeDef.BrOpen)),tokentree_TokenTreeDef.DblDot);
		if(colon != null) {
...
class formatter_marker_wrapping_MarkWrapping extends formatter_marker_wrapping_MarkWrappingBase {
	...
	markSingleMethodChain(chainStart) {
		...
		let firstMethodCall = tokentree_TokenTreeAccessHelper.matches(tokentree_TokenTreeAccessHelper.parent(tokentree_TokenTreeAccessHelper.isCIdent(tokentree_TokenTreeAccessHelper.parent(chainStart))),tokentree_TokenTreeDef.Dot);
	...
	findOpAddItemStart(itemStart) {
		...
		while(parent != null && parent.tok != tokentree_TokenTreeDef.Root) {
			...
			case 6:
				...
				break;
			case 16:
				let bkClose = tokentree_TokenTreeAccessHelper.firstOf(parent,tokentree_TokenTreeDef.BkClose);
...
class haxeparser_HaxeLexer extends hxparse_Lexer {
	...
	static inlineMarkup(lexer) {
		...
		let this1 = haxe_io_Bytes.ofString(text);
		let textBytes = this1;
	...
	static walkKeyword(stream,parent) {
		...
		if(_g._hx_index == 1) {
			...
			case 16:
				...
				return true;
			case 17:case 18:case 19:case 35:
...
codesamples_SamelineSamples.__meta__ = { obj : { sectionName : ["Sameline samples"]}, fields : { blockless_function_body : { codeSampleName : ["blockless.function.body"], keywords : [["blockless","function","body","keep","tries","linebreaks","from","input","source","sameline","functionbody","same","anonfunctionbody"]]}, if_sameline : { codeSampleName : ["if.sameline"], keywords : [["sameline","keep","tries","linebreaks","from","input","source","ifbody","next","elsebody","ifelse","same","elseif","expressionif","expressionifwithblocks","false"]]}}};
